  ...
  return (
    <div ref={ref} className="bg-[#F5F5F5] py-23">
      {/* Mobile: edge-to-edge snap strip (was a fixed-width row that overflowed
          ...
          Cloudinary uploads get e_trim + c_fill,g_auto applied server-side
          (see lib/cloudinary.ts) so the subject actually fills the source
          image before it ever reaches object-cover here — that's what fixes
          the cut/bordered look, not CSS letterboxing. Bundled /public
          fallbacks are already pre-trimmed PNGs. */}
